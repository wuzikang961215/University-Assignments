...
// avengers
/*
created by Zikang Wu
zikang.wu@student.adelaide.edu.au
2022
*/
#include <iostream>
#include <fstream>
...
int main(int argc, char *argv[])
{
    ...
    {
        // modification 3: calculating response time
        if (!queue.empty())
        {
            for (int i = 0; i < queue.size(); i++)
            {
                if (customers[queue[i]].served == false)
                    // add response time
                    customers[queue[i]].response++;
                // limit maximum response time
                if (customers[queue[i]].response > 150)
                {
                    queue.insert(queue.begin(), queue[i]);
        ...
        {
            // modification 2
            // short remaining time first algorithm
            if (queue.empty())
            {
            ...
            else
            {
                for (int i = 0; i < queue.size(); i++)
                {
                    // modification 4: priority added in if statement
                    if (customers[queue[i]].slots_remaining >= customers[arrival_events[0].customer_id].slots_remaining || customers[queue[i]].priority > customers[arrival_events[0].customer_id].priority)
                    {
                        queue.insert(queue.begin() + i, arrival_events[0].customer_id);
                        arrival_events.pop_front();
                        break;
                    }
                    // if the arrival event requires longest time
                    if (i == queue.size() - 1)
                    {
                        queue.push_back(arrival_events[0].customer_id);
                        arrival_events.pop_front();
        ...
        if (current_id == -1)
        {
            if (!queue.empty()) // is anyone waiting?
            {
                ...
                customers[current_id].response = 0;
                customers[current_id].served = true;
                queue.pop_front();
