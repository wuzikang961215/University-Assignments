...
int main(int argc, char *argv[])
{
  ...
  while (listen_socket >= 0)
  {
    ...
    if ((pid = fork()) == 0)
    {
      ...
      bool parsed_request = Parse_HTTP_Request(connection_socket, &new_request);
      /* END CODE SNIPPET 6 */
      

      /* 7) Decide which status_code and reason phrase to return to client */
      /* START CODE SNIPPET 7 */
      bool valid_resource = Is_Valid_Resource(new_request.URI);
      if(valid_resource == true){
      ...
      int symbol = 0;
      int i;
      for (i = 0; i < sizeof(new_request.method); i++){
        if (new_request.method[i] == '/' || new_request.method[i] == '.')
          symbol++;
      }        
      if (symbol > 0){
        status_code = 400;
      ...
      if(strcmp(new_request.method, "GET") == 0)
        is_ok_to_send_resource = valid_resource;
      /* END CODE SNIPPET 10 */
