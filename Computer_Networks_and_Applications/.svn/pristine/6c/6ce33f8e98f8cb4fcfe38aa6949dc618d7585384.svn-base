...
void A_output(struct msg message)
{
  ...
  if ( windowcount < WINDOWSIZE) {
    ...
    buffer[windowlast].seqnum = sendpkt.seqnum;
    windowcount++;
    ...
    /* sequence number is ranged from 0 to 6 */
    if (A_nextseqnum < 6)
      A_nextseqnum++;
    else
      A_nextseqnum = 0;
...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet)) {
    ...
    if (duplicate == false) {    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
      /* packet is a new ACK */
      if (TRACE > 0)
      ...
      int difference = pktIndex - windowfirst + 1;
      windowcount = windowcount - difference;
      for (int i = windowfirst; i <= windowlast - difference; i++)
        buffer[i] = buffer[i + difference];
      windowlast = windowlast - difference;
...
void A_timerinterrupt(void)
{
  ...
  int i;
  for ( i = windowfirst; i <= windowlast; i++){
    if (TRACE > 0)
  ...
  packets_resent = packets_resent + windowlast - windowfirst + 1;
}       
