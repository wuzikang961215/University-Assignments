...
static string callclassname ;
// function name
static string functionName ;

static int nlocals ;
static int nwhiles = 0 ;
static int nif = 0 ;
static int callfunction = 0 ;
static int paramsize = 0 ;
static int methoddec = 0 ;
static int constructordec = 0 ;
static int callmethod = 0 ;
...
void walk_var_decs(ast t)
{
    ...
    write_to_output("function " + className + "." + functionName + " " + to_string(nlocals) + "\n") ;
    if ( methoddec > 0 )
    {
        write_to_output("push argument 0\n");
        write_to_output("pop pointer 0\n");
    }
    else if ( constructordec > 0 )
    {
        write_to_output("push constant " + to_string(paramsize) + "\n");
        write_to_output("call Memory.alloc " + to_string(1) + "\n");
        write_to_output("pop pointer 0\n");
        constructordec = 0 ;
    }
}
...
void walk_if(ast t)
{
    ...
    string ifcounter = to_string(nif++);

    walk_expr(condition) ;
    write_to_output("if-goto IF_TRUE" + ifcounter + "\n");
    write_to_output("goto IF_FALSE" + ifcounter + "\n");
    write_to_output("label IF_TRUE" + ifcounter + "\n");
    walk_statements(if_true) ;
    write_to_output("label IF_FALSE" + ifcounter + "\n");
}
...
void walk_if_else(ast t)
{
    ...
    write_to_output("label IF_TRUE" + ifcounter + "\n");
    walk_statements(if_true) ;
    write_to_output("goto IF_END" + ifcounter + "\n");
    write_to_output("label IF_FALSE" + ifcounter + "\n");
    walk_statements(if_false) ;
    write_to_output("label IF_END" + ifcounter + "\n");
}
...
void walk_while(ast t)
{
    ...
    string whilecounter = to_string(nwhiles++);

    write_to_output("label WHILE_EXP" + whilecounter + "\n");
    walk_expr(condition) ;
    write_to_output("not\n");
    write_to_output("if-goto WHILE_END" + whilecounter + "\n");
    walk_statements(body) ;
    write_to_output("goto WHILE_EXP" + whilecounter + "\n");
    write_to_output("label WHILE_END" + whilecounter + "\n");
}
...
void walk_expr(ast t)
{
    ...
    for ( int i = 0 ; i < termops.size() ; i++ )
    {
        ...
        {
            walk_term(termops[i+1]) ;
            walk_infix_op(termops[i]) ;
            i++ ;
        }
    }
}
...
void walk_string(ast t)
{
    ...
    for ( int i = 0 ; i < _constant.length() ; i++ )
    {
        char a = _constant[i];
        int b = a ;
        write_to_output("push constant " + to_string(b) + "\n");
...
void walk_this(ast t)
{
    ...
    else //if ( methoddec > 0 || callmethod > 0 )
        write_to_output("push pointer 0\n");
}
...
void walk_subr_call(ast t)
{
    ...
    if ( callfunction > 0 )
        write_to_output("call " + callclassname + "." + subr_name + " " + to_string(paramsize) + "\n");
    else if ( callmethod > 0 )
        write_to_output("call " + callclassname + "." + subr_name + " " + to_string(paramsize+1) + "\n");
}
