...
void walk_var_dec(ast t)
{
    ...
    if ( get_var_dec_type(t) == "int" )
        countintvar++ ;
    else if ( get_var_dec_type(t) == "char" )
        countcharvar++ ;
    else if ( get_var_dec_type(t) == "boolean" )
        countboolvar++ ;
    vars.push_back(t) ;
}
...
void walk_var_decs(ast t)
{
    ...
    for ( int i = 0 ; i < vars.size() ; i++ )
    {
        ...
        {
            if ( countbool == 0 )
                write_to_output("        var boolean ") ;
            if ( countbool > 0 )
                write_to_output(",") ;
            write_to_output(get_var_dec_name(vars[i])) ;
                countbool++ ;
            if ( countbool == countboolvar )
                write_to_output(" ;\n") ;
        }
        if ( countcharvar > 0 )
        {
            if ( countchar == 0 )
                write_to_output("        var char ") ;
            if ( countchar > 0 )
                write_to_output(",") ;
            write_to_output(get_var_dec_name(vars[i])) ;
                countchar++ ;
            if ( countchar == countcharvar )
                write_to_output(" ;\n") ;
        }
        if ( countintvar > 0 )
        {
            if ( countint == 0 )
                write_to_output("        var int ") ;
            if ( countint > 0 )
                write_to_output(",") ;
            write_to_output(get_var_dec_name(vars[i])) ;
                countint++ ;
            if ( countint == countintvar )
                write_to_output(" ;\n") ;
        }
    }
    if ( ndecs != 0 )
