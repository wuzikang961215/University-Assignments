...
ast parse_a_instr_name()
{
    ...
    name = token_spelling(mustbe(tk_name));             // replace this with code to parse '@'name
...
ast parse_a_instr_number()
{
    ...
    ivalue = token_ivalue(mustbe(tk_number));          // replace this with code to parse '@'number
...
ast parse_c_instr()
{
    ...
    if ( have(tk_dest) ) dest = token_spelling(mustbe(tk_dest)) ;

    alu_op = token_spelling(mustbe(tk_alu_op)) ;

    if ( have(tk_jump) ) jump = token_spelling(mustbe(tk_jump)) ;
